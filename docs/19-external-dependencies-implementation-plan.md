...
| 항목 | 역할 | 왜 필요한가 | 지금 상태 |
| --- | --- | --- | --- |
| Supabase Auth | 실제 사용자 로그인/세션 | dev 로그인 버튼을 없애고 실제 회원 흐름으로 바꾸기 위해 필요 | 로컬 대체 구현 |
| Supabase Postgres | 운영 DB | 현재 로컬 Postgres를 실제 운영 DB로 바꾸기 위해 필요 | 로컬 DB 사용 중 |
| Supabase Storage | 이미지 업로드 저장소 | 지금은 URL 입력 방식이라 실제 사용자 업로드가 불가 | 미구현 |
| Resend | 트랜잭션 메일 발송 | claim 링크, 운영 상태 변경, 새 댓글 알림 발송에 필요 | 미구현 |
| Turnstile | 봇/스팸 방지 | submit, comment, report를 자동화 공격에서 보호 | 미구현 |
| Upstash Rate Limit | production-grade 속도 제한 | 현재 DB 기반 rate limit보다 가볍고 예측 가능한 보호 | 로컬 대체 구현 |
| Scheduler | 정기 작업 실행 | 랭킹 갱신, 링크 헬스체크, 미claim 정리, 메일 큐 처리에 필요 | 수동 실행만 가능 |
| Domain Verification | 도메인 소유 신뢰 배지 | 메이커 주장과 실제 서비스 소유를 구분하기 위해 필요 | 미구현 |
...
이 상태로는 실제 사용자 가입, 댓글, 저장, owner 연결을 운영 환경에서 안전하게 처리할 수 없다.

Auth가 필요한 이유는 아래와 같다.

- 댓글과 저장에 실제 사용자 식별자가 필요하다.
...
- dev 로그인 버튼 제거 가능
- 이메일 매직링크로 실제 로그인 가능
- `/me/*`, 댓글, 저장, claim이 Supabase 세션 기준으로 동작
...
문서상 claim은 이메일 경로가 핵심인데, 현재는 실제 메일 발송이 없다.
즉 지금은 링크를 브라우저에서 바로 보여주는 로컬 편의 흐름일 뿐이다.
